...
export interface ProcessMapStepContent {
  label: string;
  detail?: string;
  actor?: string;
}
...
export interface ProcessMapContent {
  description: string;
  steps: ProcessMapStepContent[];
}

export async function saveProcessMap(input: {
  projectId: string;
  phaseSlug: string;
  sectionSlug: string;
  topicSlug: string;
  content: ProcessMapContent;
}): Promise<{ ok: true; graded: boolean }> {
  const ctx = await requireAuthContext();
  const db = getAppDb();
  const path = topicPath(input.phaseSlug, input.sectionSlug, input.topicSlug);

  const cleanedSteps: ProcessMapStepContent[] = input.content.steps
    .map((s) => ({
      label: s.label.trim(),
  ...
  const cleaned: ProcessMapContent = {
  ...
  try {
    const r = await gradeTopic({
      workspaceId: ctx.workspace.id,
      userId: ctx.user.id,
      topic: { kind: "process-map", input: cleaned },
    });
    ...
    console.error("[save-process-map] inline grading failed:", err);
  }
